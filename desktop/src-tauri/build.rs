...
fn ensure_native_engine_resource_placeholder() {
    ...
    let resource_path = PathBuf::from(&manifest_dir)
        .join("resources")
        .join("cue-rs");
    let legacy_resource_path = PathBuf::from(&manifest_dir)
        .join("resources")
        .join("brood-rs");
    ...
    let _ = fs::write(&legacy_resource_path, stub);

    #[cfg(target_family = "unix")]
    {
        let _ = fs::set_permissions(&resource_path, fs::Permissions::from_mode(0o755));
        let _ = fs::set_permissions(&legacy_resource_path, fs::Permissions::from_mode(0o755));
    }
}
