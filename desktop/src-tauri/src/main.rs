...
fn native_engine_program_candidates(app: Option<&tauri::AppHandle>) -> Vec<EngineProgramCandidate> {
    ...
    for env_key in ["CUE_RS_BIN", "CUE_ENGINE_BINARY", "BROOD_RS_BIN", "BROOD_ENGINE_BINARY"] {
...
fn get_key_status() -> Result<serde_json::Value, String> {
    ...
    let canvas_provider = resolve_provider(&["CUE_CANVAS_CONTEXT_REALTIME_PROVIDER", "BROOD_CANVAS_CONTEXT_REALTIME_PROVIDER"]);
    let intent_provider = resolve_provider(&["CUE_INTENT_REALTIME_PROVIDER", "BROOD_INTENT_REALTIME_PROVIDER"]);
    let mother_intent_provider = resolve_provider(&["CUE_MOTHER_INTENT_REALTIME_PROVIDER", "BROOD_MOTHER_INTENT_REALTIME_PROVIDER"]);
...
fn get_install_telemetry_defaults() -> Result<serde_json::Value, String> {
    ...
    let opt_in = env_flag(env_value(&vars, &["CUE_INSTALL_TELEMETRY", "BROOD_INSTALL_TELEMETRY"]))
        .or_else(|| env_flag(env_value(&vars, &["CUE_TELEMETRY", "BROOD_TELEMETRY"])))
        .or(disk_opt_in)
        .unwrap_or(false);
    let force_opt_in = env_flag(env_value(&vars, &["CUE_INSTALL_TELEMETRY_FORCE", "BROOD_INSTALL_TELEMETRY_FORCE"]))
        .or(disk_force_opt_in)
        .unwrap_or(false);
    let endpoint = env_value(&vars, &["CUE_INSTALL_TELEMETRY_ENDPOINT", "BROOD_INSTALL_TELEMETRY_ENDPOINT"])
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
        .or(disk_endpoint);
    let install_id = env_value(&vars, &["CUE_INSTALL_TELEMETRY_INSTALL_ID", "BROOD_INSTALL_TELEMETRY_INSTALL_ID"])
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
        .or(disk_install_id);
...
fn start_external_bridge(state: SharedPtyState, app_handle: tauri::AppHandle) {
    #[cfg(target_family = "unix")]
    {
        let socket = first_non_empty_env(&["CUE_DESKTOP_BRIDGE_SOCKET", "BROOD_DESKTOP_BRIDGE_SOCKET", "BROOD_BRIDGE_SOCKET"])
