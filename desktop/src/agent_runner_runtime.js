...
import { resolveSingleImageAffordanceRoute } from "./single_image_capability_routing.js";
...
function summarizeSessionTools(sessionTools = []) {
  return (Array.isArray(sessionTools) ? sessionTools : []).slice(0, 8).map((tool) => ({
    ...
    ...(readFirstString(tool?.execution?.executionType, tool?.executionType)
      ? { executionType: readFirstString(tool?.execution?.executionType, tool?.executionType) }
      : {}),
    ...(readFirstString(tool?.execution?.routeProfile, tool?.routeProfile)
      ? { routeProfile: readFirstString(tool?.execution?.routeProfile, tool?.routeProfile) }
      : {}),
    ...(readFirstString(tool?.execution?.routingStrategy, tool?.routingStrategy)
      ? { routingStrategy: readFirstString(tool?.execution?.routingStrategy, tool?.routingStrategy) }
      : {}),
    ...((asRecord(tool?.execution?.localRuntime) || asRecord(tool?.localRuntime))
      ? { localRuntime: cloneJson(asRecord(tool?.execution?.localRuntime) || asRecord(tool?.localRuntime)) }
      : {}),
...
function summarizeDirectAffordanceStates() {
  return Array.from(DIRECT_AFFORDANCE_IDS)
    .map((toolId) => {
      const route = resolveSingleImageAffordanceRoute({ jobId: toolId });
...
export function buildAgentRunnerContextSummary({
  ...
  return {
    ...
    availableActions: {
      ...
      directAffordanceStates: summarizeDirectAffordanceStates(),
