...
export function buildAgentRunnerPlannerPrompt(input = {}) {
  ...
  return [
    ...
    "15. If goalContract.hardRequirements exist, do not treat palette shifts, props, uniforms, or single-subject styling as sufficient when a required named entity, object, scene cue, or interaction is still missing.",
    "16. Use request_design_review when the goal is aesthetic, ambiguous, or multi-step and no direct single-image action can complete it.",
    "17. Use accept_review_proposal only if review status is ready and a proposal is available.",
    "18. If review proposals only improve soft style cues while hard requirements remain unmet, continue planning instead of stopping.",
    "19. Use create_tool only when a reusable local pattern is clearly warranted.",
    "20. Export or stop only when the visible canvas satisfies the hard requirements in goalContract or, if no hard requirements were extracted, the goal appears visibly satisfied.",
    "",
    "Action schema:",
    '{',
