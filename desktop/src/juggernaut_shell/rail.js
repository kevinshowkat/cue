...
const VISIBLE_COMMUNICATION_SHELL_TOOL_IDS = new Set(["protect"]);
...
function syncExternalRailState(root) {
  ...
  for (const button of Array.from(root.querySelectorAll("button[data-tool-id]"))) {
    ...
    const isSelected = toolId === "protect" && communicationTool === "protect";
    if (VISIBLE_COMMUNICATION_SHELL_TOOL_IDS.has(toolId)) {
