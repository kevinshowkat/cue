export const SESSION_TAB_SCHEMA_VERSION = "session-tab-v1";
...
export const SESSION_TABS_BRIDGE_KEY = TABBED_SESSIONS_BRIDGE_KEY;
export const SESSION_TABS_CHANGED_EVENT = TABBED_SESSIONS_CHANGED_EVENT;
...
const TAB_METADATA_KEYS = new Set([
  ...TAB_METADATA_FIELDS,
  "schemaVersion",
  "title",
  "active",
...
function readAliasedDescriptorValue(object, descriptors, keys = []) {
  for (const key of keys) {
    const resolved = readDescriptorValue(object, descriptors, key);
...
function cloneTabSummary(tab, activeTabId, totalTabs) {
  ...
  const title = normalizeTabLabel(meta?.label || tab?.title || tab?.label, tabId);
  const isActive = tabId === activeTabId;
  const isBusy = Boolean(meta?.busy ?? tab?.isBusy ?? tab?.busy);
  const isDirty = Boolean(meta?.dirty ?? tab?.isDirty ?? tab?.dirty);
  return {
...
function applyTabMetadata(record, input, descriptors, { tabId, now, isInsert = false }) {
  ...
  const label = readAliasedDescriptorValue(input, descriptors, ["title", "label"]);
  const runDir = readDescriptorValue(input, descriptors, "runDir");
  const busy = readAliasedDescriptorValue(input, descriptors, ["isBusy", "busy"]);
  const dirty = readAliasedDescriptorValue(input, descriptors, ["isDirty", "dirty"]);
...
export function createTabbedSessionsStore({ onChange = null } = {}) {
  ...
  function ensureTabRecord(tabId) {
    ...
    Object.defineProperty(record, "schemaVersion", {
      ...
      configurable: true,
      get() {
        return SESSION_TAB_SCHEMA_VERSION;
      },
    });
    ...
    Object.defineProperty(record, "active", {
      enumerable: false,
      configurable: true,
      get() {
        return meta.tabId === activeTabId;
      },
    });
    Object.defineProperty(record, "isActive", {
      enumerable: false,
      configurable: true,
      get() {
        return meta.tabId === activeTabId;
    ...
    Object.defineProperty(record, "isBusy", {
      enumerable: false,
      configurable: true,
      get() {
        return meta.busy;
  ...
  function upsertTab(tab, { activate = false, index = tabsOrder.length } = {}) {
    ...
    const activeInput = readAliasedDescriptorValue(tab, descriptors, ["isActive", "active"]);
    ...
    if ((activeInput.found && Boolean(activeInput.value)) || activate || (!activeTabId && tabsOrder.includes(tabId))) {
