...
async function applyDeterministicLocalToolRequest(request = {}, host = {}) {
  ...
  try {
    ...
    if (plan.capability) meta.capability = plan.capability;
    if (plan.executionType) meta.executionType = plan.executionType;
    if (plan.routeProfile) meta.routeProfile = plan.routeProfile;
    if (plan.routingStrategy) meta.routingStrategy = plan.routingStrategy;
    if (plan.localRuntime) meta.localRuntime = cloneJson(plan.localRuntime);
