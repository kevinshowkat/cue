...
export function normalizeToolManifest(raw = {}, { existingIds = [] } = {}) {
  ...
  const executionCapability = readFirstString(execution?.capability, raw?.capability) || null;
  const executionType = readFirstString(execution?.executionType, raw?.executionType) || null;
  const routeProfile = readFirstString(execution?.routeProfile, raw?.routeProfile) || null;
  const routingStrategy = readFirstString(execution?.routingStrategy, raw?.routingStrategy) || null;
  const localRuntime = asRecord(execution?.localRuntime) || asRecord(raw?.localRuntime);
  ...
  if (executionCapability) manifestExecution.capability = executionCapability;
  if (executionType) manifestExecution.executionType = executionType;
  if (routeProfile) manifestExecution.routeProfile = routeProfile;
  if (routingStrategy) manifestExecution.routingStrategy = routingStrategy;
  if (localRuntime) manifestExecution.localRuntime = cloneJson(localRuntime);
  const manifest = {
    ...
    provenance: resolveActionProvenance({
      executionType: manifestExecution.executionType || "",
      executionKind: manifestExecution.kind,
...
export function buildToolInvocation(
  ...
  return {
    ...
    tool: {
      ...
      ...(normalizedManifest.execution.executionType
        ? { executionType: normalizedManifest.execution.executionType }
        : {}),
      ...(normalizedManifest.execution.capability
        ? { capability: normalizedManifest.execution.capability }
        : {}),
      ...(normalizedManifest.execution.routeProfile
        ? { routeProfile: normalizedManifest.execution.routeProfile }
        : {}),
      ...(normalizedManifest.execution.routingStrategy
        ? { routingStrategy: normalizedManifest.execution.routingStrategy }
        : {}),
      ...(normalizedManifest.execution.localRuntime
        ? { localRuntime: cloneJson(normalizedManifest.execution.localRuntime) }
        : {}),
...
function buildSingleImageAffordanceToolDescriptor(route) {
  return {
    ...
    localRuntime: cloneJson(route.localRuntime || null),
...
export function buildSingleImageDirectAffordanceInvocation(
  ...
  return {
    ...
    receipt: {
      manifestSchema: SINGLE_IMAGE_RAIL_CONTRACT,
      reproducible: true,
      include: ["capability", "selection", "execution", "routeProfile", "routingStrategy", "localRuntime"],
    },
    failureBehavior: {
      kind: "toast",
