...
test("startup boot overlay points at the local Desktop MP4 and gates reveal until boot finishes", () => {
  ...
  assert.match(html, /html,\s*body\s*\{\s*background: #eee9db;/);
  assert.match(html, /const LOCAL_BOOT_VIDEO_PATH = "";/);
  assert.doesNotMatch(html, /\/Users\/mainframe\/Desktop\/load\.mp4/);
  assert.match(html, /const LEGACY_BOOT_VIDEO_SESSION_KEY = "juggernaut\.localBootVideoShown";/);
  assert.match(html, /const LOCAL_BOOT_VIDEO_SESSION_KEY = "cue\.localBootVideoShown";/);
  ...
  assert.match(
    html,
    /window\.sessionStorage\?\.getItem\(LOCAL_BOOT_VIDEO_SESSION_KEY\) === "1" \|\|[\s\S]*window\.sessionStorage\?\.getItem\(LEGACY_BOOT_VIDEO_SESSION_KEY\) === "1"/
  );
  assert.match(html, /window\.sessionStorage\?\.setItem\(LOCAL_BOOT_VIDEO_SESSION_KEY, "1"\);/);
  assert.match(html, /window\.sessionStorage\?\.removeItem\(LEGACY_BOOT_VIDEO_SESSION_KEY\);/);
  ...
  assert.match(html, /window\.__cue_booted = true;/);
...
test("startup boot overlay keeps a restrained centered boot video and fade-out dismissal", () => {
  ...
  assert.match(css, /background:\s*#eee9db;/);
