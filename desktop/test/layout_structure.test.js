...
test("Juggernaut shell: stage chrome carries the left rail, custom tools, and workspace header while legacy dock markup remains available", () => {
  ...
  assert.match(html, /class=\"brand-strip\"[^>]*role=\"toolbar\"[\s\S]*class=\"juggernaut-brand\"[^>]*data-tauri-drag-region[\s\S]*class=\"window-drag-region\"[^>]*data-tauri-drag-region[\s\S]*id=\"session-tab-strip\"[\s\S]*class=\"session-tab-run\"[\s\S]*id=\"session-tab-list\"[\s\S]*id=\"session-tab-new\"[\s\S]*id=\"session-tab-fork\"[\s\S]*id=\"session-tab-history\"[\s\S]*id=\"juggernaut-agent-runner-open\"[\s\S]*id=\"session-tab-design-review\"[\s\S]*id=\"juggernaut-export-psd\"[\s\S]*class=\"sr-only\"[\s\S]*id=\"engine-status\"[\s\S]*id=\"mother-intent-source-indicator\"[\s\S]*id=\"app-menu-toggle\"/);
