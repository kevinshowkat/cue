...
test("OpenRouter onboarding restart polls the typed session status reader until the engine is ready", () => {
  assert.match(
    app,
    /async function restartEngineAfterOpenRouterKeySave\(\) \{[\s\S]*readPtyStatus\(\{ useCache: false \}\)[\s\S]*normalizePtyStatus\(status\)[\s\S]*engine did not report ready after restart/
  );
});
