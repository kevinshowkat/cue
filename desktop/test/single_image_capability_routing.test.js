  ...
  assert.deepEqual(
    Object.fromEntries(
      Object.entries(SINGLE_IMAGE_DIRECT_AFFORDANCE_MAP).map(([jobId, spec]) => [
        jobId,
        {
          ...
          localRuntime: spec.localRuntime,
          localOperation: spec.localOperation,
          localRuntimeTarget: spec.localRuntime?.target || null,
    ...
    ),
    {
      ...
      polish: {
        ...
        localRuntime: expectedLocalRuntime,
...
test("single-image direct affordance routing keeps polish local-first and escalates relight to model when directionality is requested", () => {
  ...
  assert.deepEqual(
    {
      ...
      routingStrategy: polish.routingStrategy,
      localRuntime: polish.localRuntime,
      localOperation: polish.localOperation,
      provenance: polish.provenance,
      localRuntimeTarget: polish.localRuntime?.target,
      localRuntimeOrder: polish.localRuntime?.resolutionOrder,
    },
    {
      jobId: "polish",
      ...
      provenance: "local_first",
      localRuntimeTarget: "single_image_local_edit",
      localRuntimeOrder: ["installed_pack_manifest", "cue_home_env", "cue_env", "legacy_env"],
    }
  );
  ...
  assert.equal(availability.routingStrategy, "local_first_with_model_fallback");
  assert.equal(availability.localRuntime?.target, "single_image_local_edit");
  assert.equal(availability.provenance, "local_first");
  assert.equal(availability.localRuntime?.baselinePlatform, "macos");
  assert.equal(availability.localRuntime?.windowsStatus, "secondary");
});

test("single-image direct affordance routing preserves host-provided local runtime metadata verbatim", () => {
  ...
  assert.equal(route.executionKind, "model_capability");
  assert.equal(route.routingStrategy, "local_first_with_model_fallback");
  assert.deepEqual(route.localRuntime, {
    target: "single_image_local_edit",
    resolutionOrder: ["installed_pack_manifest", "cue_home_env", "cue_env", "legacy_env"],
    available: false,
    disabledReason: "capability_unavailable",
...
test("single-image local-first routes fall back to model execution when the local runtime is unavailable but the provider path still exists", () => {
  const fallbackRoute = resolveSingleImageAffordanceRoute(
    {
      jobId: "polish",
      params: {
        intensity: 0.5,
  ...
  assert.equal(fallbackRoute.executionKind, "model_capability");
  assert.equal(fallbackRoute.routingStrategy, "local_first_with_model_fallback");
  assert.equal(fallbackRoute.localRuntime?.packId, "cue.image-polish");
  assert.equal(fallbackRoute.localRuntime?.packVersion, "0.1.0");
  assert.equal(fallbackRoute.localRuntime?.resolutionSource, "installed_pack_manifest");
  ...
  const availability = resolveSingleImageCapabilityAvailability(
    ...
    {
      activeImageId: "img-12",
      selectedImageIds: ["img-12"],
      localExecutorAvailable: false,
      localRuntimeAvailability: {
        image_polish: {
          available: false,
  ...
  assert.equal(availability.enabled, true);
  assert.equal(availability.executionKind, "model_capability");
  assert.equal(availability.routingStrategy, "local_first_with_model_fallback");
});
