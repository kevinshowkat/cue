...
test("Session tab strip seeds a visible launch tab and keeps history on a dedicated shelf below the titlebar", () => {
  assert.match(
    html,
    /id=\"session-tab-list\"[^>]*role=\"tablist\"[^>]*aria-label=\"Open sessions\"[^>]*data-session-tab-schema=\"session-tab-v1\"[^>]*>[\s\S]*data-schema-version=\"session-tab-v1\"[\s\S]*data-tab-id=\"tab-launch\"[\s\S]*<span class=\"session-tab-title\">Untitled Canvas<\/span>[\s\S]*<\/div>/
  );
  assert.match(
    html,
...
test("Cue menu mirrors the session lifecycle surface and excludes dropped shell toggles", () => {
  assert.match(
    html,
    /id=\"app-menu-toggle\"[^>]*aria-label=\"Cue menu\"[\s\S]*id=\"app-menu\"[^>]*role=\"menu\"[^>]*aria-label=\"Cue menu\"[\s\S]*id=\"new-run\"[^>]*>New Session<\/button>[\s\S]*id=\"open-run\"[^>]*>Open Session…<\/button>[\s\S]*id=\"save-session\"[^>]*>Save Session<\/button>[\s\S]*id=\"close-session\"[^>]*>Close Session<\/button>[\s\S]*id=\"import\"[^>]*>Import Photos…<\/button>[\s\S]*id=\"export\"[^>]*>Export…<\/button>[\s\S]*id=\"settings-toggle\"[^>]*>Settings…<\/button>/
  );
  assert.doesNotMatch(html, /auto-accept-suggested-ability-toggle/);
});
