...
test("Session tab strip mounts inside the titlebar brand strip with core-owned ids", () => {
  assert.match(
    html,
    /class=\"brand-strip\"[^>]*role=\"toolbar\"[\s\S]*class=\"window-drag-region\"[^>]*data-tauri-drag-region[\s\S]*id=\"session-tab-strip\"[\s\S]*class=\"session-tab-run\"[\s\S]*id=\"session-tab-list\"[^>]*role=\"tablist\"[\s\S]*id=\"session-tab-new\"[\s\S]*id=\"session-tab-fork\"[\s\S]*id=\"session-tab-history\"[\s\S]*id=\"session-tab-design-review\"/
  );
...
test("Session tab strip seeds a visible launch tab and exposes History plus the Design Review action", () => {
  ...
  assert.match(
    html,
    /id=\"session-tab-history\"[^>]*class=\"session-tab-strip-action session-tab-runtime-action session-tab-history-action\"[^>]*aria-label=\"History\"[^>]*aria-controls=\"timeline-dock\"[^>]*aria-expanded=\"true\"[^>]*aria-pressed=\"true\"[^>]*title=\"History\"[\s\S]*data-juggernaut-icon-slot=\"history\"[\s\S]*<span>History<\/span>/
  );
  assert.match(
    html,
  ...
  assert.match(
    html,
    /id=\"juggernaut-agent-runner-open\"[^>]*class=\"session-tab-strip-action session-tab-runtime-action\"[^>]*title=\"Agent Run\"[\s\S]*data-juggernaut-icon-slot=\"agent_run\"[\s\S]*<span>Agent Run<\/span>/
  );
  assert.match(
    html,
    /id=\"juggernaut-export-psd\"[^>]*class=\"session-tab-strip-action session-tab-runtime-action session-tab-action-menu-toggle\"[^>]*title=\"Export\"[\s\S]*data-juggernaut-icon-slot=\"export\"[\s\S]*<span>Export<\/span>/
  );
  assert.match(
    html,
  ...
  assert.match(
    brandStripChunk,
    /class=\"session-tab-run\"[\s\S]*id=\"session-tab-list\"[\s\S]*id=\"session-tab-new\"[\s\S]*id=\"session-tab-fork\"[\s\S]*class=\"session-tab-strip-actions\"[\s\S]*id=\"session-tab-history\"[\s\S]*id=\"juggernaut-agent-runner-open\"[\s\S]*id=\"juggernaut-export-psd\"[\s\S]*id=\"session-tab-design-review\"/
  );
