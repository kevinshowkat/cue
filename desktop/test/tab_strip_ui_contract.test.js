...
test("Session tab strip CSS keeps the strip compact, scrollable, and stateful", () => {
  ...
  assert.match(css, /body\.juggernaut-shell \.brand-strip \.session-tab-strip-action\.is-external-model,\s*body\.juggernaut-shell \.brand-strip \.session-tab-runtime-action\.is-external-model\s*\{[\s\S]*var\(--jg-pack-accent-strong\)[\s\S]*var\(--jg-pack-paper-soft\)/);
  assert.match(css, /body\.juggernaut-shell \.brand-strip \.session-tab-strip-action\.is-external-model span,\s*body\.juggernaut-shell \.brand-strip \.session-tab-strip-action\.is-external-model \.session-tab-action-icon \.tool-icon\s*\{[\s\S]*color:\s*var\(--jg-pack-ink\);/);
  assert.match(css, /body\.juggernaut-shell \.brand-strip \.session-tab-strip-action\.is-external-model:active,\s*body\.juggernaut-shell \.brand-strip \.session-tab-runtime-action\.is-external-model\.is-active-request\s*\{[\s\S]*var\(--jg-pack-secondary-soft\)[\s\S]*transform:\s*translateY\(1px\);/);
