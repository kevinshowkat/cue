...
test("session reopen helpers prefer canonical session.json writes and keep legacy juggernaut-session.json as a read fallback", () => {
  ...
  assert.match(
    app,
    /async function sessionSnapshotPathForRunDir\(runDir = ""\) \{[\s\S]*SESSION_SNAPSHOT_FILENAME/
  );
  assert.match(
    app,
    /async function legacySessionSnapshotPathForRunDir\(runDir = ""\) \{[\s\S]*LEGACY_SESSION_SNAPSHOT_FILENAME/
  );
  assert.match(
    app,
...
test("inactive tabs drop live polling and realtime engine attachments in v1", () => {
  ...
  assert.match(
    app,
    /if \(update\.kind === DESKTOP_SESSION_UPDATE_KINDS\.EVENT && update\.event\) \{[\s\S]*await handleEvent\(update\.event\);[\s\S]*\}/
  );
