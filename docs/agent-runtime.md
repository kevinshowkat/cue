...
- Agent Run always plans against the current active tab and its current bridge-backed `runDir`.
...
- Local-first direct affordance invocations and deterministic local replay should preserve `executionType`, `routeProfile`, `routingStrategy`, and `localRuntime` in invocation and receipt data whenever that metadata exists.
- Tool receipts land beside the active tab's run artifacts and should stay compatible with the current `cue.session.v1`, `cue.timeline.v1`, and receipt-lineage contracts.
...
- `invoke_direct_affordance` can carry local-first routing metadata for `polish` and `relight`.
- The current launch-slice route envelope is:
  - `executionType`
  - `routeProfile`
  - `routingStrategy`
  - `localRuntime`
- `Remove People` stays provider-only.
- `Polish` and `Relight` stay local-first with model fallback, and their `localRuntime` metadata should survive deterministic replay and in-session reuse instead of being recomputed later from UI state alone.
...
- Tool apply and deterministic local replay: `desktop/src/tool_apply_runtime.js`, `desktop/src/local_tool_edits.js`
