...
## Problem
Cue can feel reactive: the user edits the canvas, then decides what to do next. For a "desktop Image IDE", we want Cue to be one step ahead by continuously extracting lightweight context from the current canvas without slowing UX.

## UX
- Settings now includes an **Always-On Vision** toggle.
- When enabled, Cue periodically (and quietly) scans an optimized snapshot of the current canvas in the background.
...
- `rust_engine/crates/cue-contracts/src/chat/intent_parser.rs`
- `rust_engine/crates/cue-cli/src/lib.rs` (batch + realtime slash handlers)
...
- Inference: `rust_engine/crates/cue-cli/src/lib.rs` (`/canvas_context` native handler)
...
- Implementation: `rust_engine/crates/cue-cli/src/lib.rs` (native realtime session wiring)
