...
## Problem
When exactly two imported images are loaded, Cue renders an attention-grabbing "SUGGESTED: COMBINE" overlay on the canvas and also emits a tip/toast suggesting Combine. This reads like a button overlaying the images and competes with the user's own intent.

## UX
- When 2 images are loaded, Cue still supports `Multi view` and the 2-image Abilities.
