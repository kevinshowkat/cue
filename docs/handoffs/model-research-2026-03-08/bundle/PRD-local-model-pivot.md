...
- The next implementation priority after the single-image rail work is local-only support for the single-image wedge.
- First local image model target: `stabilityai/stable-diffusion-2-inpainting` via an Apple `ml-stable-diffusion` / Core ML deployment path.
- First local-only image pass prioritizes `targeted_remove`, `background_replace`, `identity_preserving_variation`, and `crop_or_outpaint`.
- `subject_isolation` stays in the seeded single-image rail contract but is out of scope for the first local inpainting pass unless a separate local segmentation path lands.
...
- The main editing loop must remain provider- and model-agnostic, including the seeded rail and its job states.
- Provider and model names must stay out of the main editing loop and primary rail.
- Provider and model details may appear in settings, diagnostics, and receipts.
...
- Local-only mode is the next implementation priority after the single-image rail phase.
- The first local image model target for this phase is `stabilityai/stable-diffusion-2-inpainting` via an Apple `ml-stable-diffusion` / Core ML deployment path.
- The first offline-priority capability set is:
  - `targeted_remove`
  - `background_replace`
  - `identity_preserving_variation`
  - `crop_or_outpaint`
- `subject_isolation` is out of scope for the first local inpainting pass unless a separate local segmentation path lands.
- In local-only mode, cloud-only tools are visibly disabled or swapped for local equivalents.
- Unavailable capabilities remain visible with the generic disabled reasons defined by the rail contract.
- Local-only mode still supports a meaningful subset of the core workflow:
  - import
  - canvas transforms
  - offline-capable seeded single-image jobs from the first local capability set when local readiness requirements are met
  - receipt generation
  - local export
- `Create Tool` remains a secondary follow-on capability and does not return to the primary rail for this phase.
...
- The main editing loop stays capability-first and must not expose provider or model names in the primary rail.
- First-party provider targets:
  - OpenAI
  - Google
  - Flux
- First local image-model implementation target for the single-image local-only wave:
  - `stabilityai/stable-diffusion-2-inpainting` via Apple `ml-stable-diffusion` / Core ML conversion
...
- A user can enable local-only mode.
- The app includes a local-only mode that works without internet access.
- The app can surface local model readiness and status for offline-capable jobs.
- Offline-capable seeded jobs backed by `targeted_remove`, `background_replace`, `identity_preserving_variation`, and `crop_or_outpaint` are executable without internet when the local model is ready.
- `Cut Out` may remain visible but disabled if no local segmentation model exists.
- The main editing loop and primary rail remain provider- and model-agnostic even when a local route is active.
...
- Receipts record local provider and model execution details when local-only mode or local routing is used.
...
### Next Phase: Local-First Single-Image Mode
- Make local-only support the next implementation priority after the single-image rail work.
- Use the attached feasibility notes and current official evidence to choose the first viable local image model path instead of defaulting to a preselected runtime.
- Execute the first offline-priority capability set: `targeted_remove`, `background_replace`, `identity_preserving_variation`, and `crop_or_outpaint`.
- Keep `subject_isolation` out of scope for this local pass unless a separate local segmentation path lands.
- Preserve the `single-image-rail-v1` contract and keep the main rail provider- and model-agnostic.
- Keep `Create Tool` in a secondary follow-on surface rather than promoting it back into the primary rail.
