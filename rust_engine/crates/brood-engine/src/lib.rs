...
fn pricing_override_path() -> Option<PathBuf> {
    ...
    if let Some(raw) = first_non_empty_env(&["CUE_HOME_DIR", "BROOD_HOME_DIR"]) {
        candidates.push(PathBuf::from(raw).join("pricing_overrides.json"));
    }
    if let Some(home) = env::var_os("HOME").map(PathBuf::from) {
        candidates.push(home.join(".cue").join("pricing_overrides.json"));
        candidates.push(home.join(".brood").join("pricing_overrides.json"));
    }
    candidates
        .into_iter()
        ...
        .or_else(|| {
            env::var_os("HOME")
                .map(PathBuf::from)
                .map(|home| home.join(".cue").join("pricing_overrides.json"))
        })
}
...
mod tests {
    ...
    #[test]
    fn default_registry_keeps_cue_supported_image_providers() {
        let providers = default_provider_registry().names();
        assert!(providers.iter().any(|name| name == "openai"));
        assert!(providers.iter().any(|name| name == "gemini"));
        assert!(providers.iter().any(|name| name == "imagen"));
        assert!(providers.iter().any(|name| name == "flux"));
        assert!(providers.iter().any(|name| name == "dryrun"));
    }
