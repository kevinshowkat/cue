...
SMOKE_OUTPUT_DIR="${SMOKE_OUTPUT_DIR:-$REPO_ROOT/outputs/verification/smoke.macos_clean_machine}"
SMOKE_LOG_PATH="${SMOKE_LOG_PATH:-$SMOKE_OUTPUT_DIR/smoke.log}"
...
SMOKE_QUEUE_ID="${SMOKE_QUEUE_ID:-smoke.macos_clean_machine}"
SMOKE_MILESTONE="${SMOKE_MILESTONE:-macos_screenshot_polish_baseline}"
...
run_proposal_flow_automation() {
  ...
  log "Automation: import second image for proposal flow"
  ...
  log "Automation: request next proposal"
  ...
  if [[ -z "$proposal_response" ]]; then
    proposal_warn_or_fail "Proposal request failed: <empty response>"
    return $?
  fi
  if ! jq -e '.ok == true' <<<"$proposal_response" >/dev/null 2>&1; then
    warn "Proposal request did not advance immediately: $proposal_response"
  fi

  log "Automation: confirm suggestion (begin proposal draft)"
  local draft_request
  draft_request="$(jq -nc '{
    op: "automation",
  ...
  if [[ -z "$draft_response" ]]; then
    proposal_warn_or_fail "Proposal draft-start confirm failed: <empty response>"
    return $?
  fi
  if ! jq -e '.ok == true' <<<"$draft_response" >/dev/null 2>&1; then
    warn "Proposal draft-start confirm did not complete cleanly: $draft_response"
  fi
  ...
  if [[ "$CUE_INSTALL_TELEMETRY" == "1" ]]; then
    ...
    if wait_for_telemetry_event "first_proposal_proposed" "$PROPOSAL_EVENT_TIMEOUT_SECONDS"; then
      ...
      log "Automation: inject deterministic local proposal draft fallback"
      ...
      if [[ -z "$inject_response" ]]; then
        proposal_warn_or_fail "Proposal local draft injection failed: <empty response>"
        return $?
      fi
      if ! jq -e '.ok == true' <<<"$inject_response" >/dev/null 2>&1; then
        warn "Proposal local draft injection did not complete cleanly: $inject_response"
      fi
      if wait_for_telemetry_event "first_proposal_proposed" "$PROPOSAL_EVENT_TIMEOUT_SECONDS"; then
        proposal_seen=1
      fi
  ...
  log "Automation: confirm suggestion (accept proposal)"
  ...
  if [[ -z "$accept_response" ]]; then
    proposal_warn_or_fail "Proposal accept failed: <empty response>"
    return $?
  fi
  if ! jq -e '.ok == true' <<<"$accept_response" >/dev/null 2>&1; then
    warn "Proposal accept did not complete cleanly: $accept_response"
  fi
  ...
  if [[ "$CUE_INSTALL_TELEMETRY" == "1" ]]; then
    ...
    if ! wait_for_telemetry_event "first_proposal_accepted" "$PROPOSAL_EVENT_TIMEOUT_SECONDS"; then
      log "Automation: retry proposal accept once"
      ...
      if [[ -n "$accept_retry_response" ]] && ! jq -e '.ok == true' <<<"$accept_retry_response" >/dev/null 2>&1; then
        warn "Proposal accept retry did not complete cleanly: $accept_retry_response"
      fi
      if ! wait_for_telemetry_event "first_proposal_accepted" "$PROPOSAL_EVENT_TIMEOUT_SECONDS"; then
        if [[ "$REQUIRE_PROPOSAL_EVENTS" == "1" ]]; then
...
log "Verification queue: $SMOKE_QUEUE_ID"
log "Milestone gate: $SMOKE_MILESTONE"
log "Artifact log: $SMOKE_LOG_PATH"
log "Coverage: install DMG, launch Cue, import a sample image, run one deterministic ability, and exercise proposal flow automation when enabled"
